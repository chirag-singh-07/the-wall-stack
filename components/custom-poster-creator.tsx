...
const frameStyles = [
  ...
  { id: "thin-black", name: "Thin Black", border: "2px solid #000", padding: "8px" },
  { id: "thick-black", name: "Thick Black", border: "8px solid #000", padding: "12px" },
  { id: "double", name: "Double Line", border: "4px double #000", padding: "10px" },
  { id: "shadow", name: "Shadow Box", border: "1px solid #000", padding: "16px" },
...
const imageFilters = [
  ...
  { id: "contrast", name: "High Contrast", filter: "grayscale(100%) contrast(150%)" },
  { id: "sepia", name: "Vintage", filter: "sepia(100%)" },
  { id: "bright", name: "Bright", filter: "brightness(120%) contrast(110%)" },
  { id: "noir", name: "Noir", filter: "grayscale(100%) contrast(200%) brightness(80%)" },
]
...
export function CustomPosterCreator() {
  ...
  const [textPosition, setTextPosition] = useState<"top" | "bottom" | "none">("none")
  const [zoom, setZoom] = useState(1)
  const [position, setPosition] = useState({ x: 0, y: 0 })
  const [isDragging, setIsDragging] = useState(false)
  const [dragStart, setDragStart] = useState({ x: 0, y: 0 })
  const [activeTab, setActiveTab] = useState<"upload" | "frame" | "filter" | "size" | "text">("upload")
  ...
  return (
    <section ref={sectionRef} className="py-24 md:py-32 bg-foreground text-background overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 border border-background/20 rounded-full mb-6">
            ...
            <span className="text-sm tracking-wider uppercase">Create Your Own</span>
          </div>
          ...
          <p className="text-background/60 max-w-2xl mx-auto text-lg">
            Transform your favorite photos into stunning wall art. Upload, customize, and order your unique poster in
            minutes.
          </p>
        </div>
          ...
          <div className="order-2 lg:order-1">
            <div className="sticky top-24">
              <div className="relative aspect-[3/4] bg-background/5 rounded-lg overflow-hidden border border-background/10">
                ...
                {/* Poster Preview */}
                <div ref={previewRef} className="absolute inset-8 md:inset-12 flex items-center justify-center">
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: selectedFrame.id === "shadow" ? "8px 8px 0 rgba(0,0,0,0.8)" : "none",
                    ...
                    {uploadedImage ? (
                      ...
                      >
                        ...
                        {textPosition !== "none" && customText && (
                          ...
                          >
                            <p className="text-sm md:text-base font-medium tracking-wider uppercase">{customText}</p>
                          </div>
                        )}
                      </div>
                    ) : (
                ...
                {uploadedImage && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-foreground/90 rounded-full px-3 py-2">
                    ...
                    <button onClick={resetImage} className="p-1 hover:bg-background/10 rounded-full transition-colors">
              ...
              {uploadedImage && (
                <div className="mt-6 p-6 border border-background/20 rounded-lg bg-background/5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-background/60">Your Custom Poster</span>
                    <span className="text-2xl font-light">${selectedSize.price}</span>
                  </div>
            ...
            <div className="space-y-8">
              ...
              {activeTab === "upload" && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  <h3 className="text-xl font-medium mb-4">Upload Your Image</h3>
                  <p className="text-background/60 mb-6">
                    Choose a high-resolution image for the best print quality. We recommend at least 2000x3000 pixels.
                  </p>
                  <div
                    onDrop={handleDrop}
                  ...
                  >
                    ...
                    <p className="text-background/80 mb-2">Drag and drop your image here</p>
                    <p className="text-sm text-background/40">or click to browse files</p>
                    <p className="text-xs text-background/30 mt-4">Supports: JPG, PNG, WebP (Max 20MB)</p>
                  </div>
              ...
              {activeTab === "frame" && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  <h3 className="text-xl font-medium mb-4">Choose Frame Style</h3>
                  <p className="text-background/60 mb-6">Select a frame style that complements your image.</p>
                    ...
                    {frameStyles.map((frame, index) => (
                      ...
                      >
                        <div
                          ...
                          style={{
                            border: frame.border.replace("#000", selectedFrame.id === frame.id ? "#000" : "#fff"),
                          }}
                        />
                        <span className="text-xs tracking-wider uppercase">{frame.name}</span>
                        {selectedFrame.id === frame.id && (
              ...
              {activeTab === "filter" && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  ...
                  <p className="text-background/60 mb-6">Enhance your image with our curated filters.</p>
                    ...
                    {imageFilters.map((filter, index) => (
                      <button
                        ...
                        className={`relative p-2 rounded-lg transition-all duration-300 ${
                          selectedFilter.id === filter.id ? "ring-2 ring-background" : "hover:bg-background/10"
                        }`}
                        style={{ animationDelay: `${index * 50}ms` }}
                      >
                        ...
                        <span className="text-xs tracking-wider uppercase">{filter.name}</span>
              ...
              {activeTab === "size" && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  ...
                  <p className="text-background/60 mb-6">Choose the perfect size for your space.</p>
                        ...
                        <div className="flex items-center gap-4">
                          <div
                            className={`flex items-center justify-center rounded ${
                              selectedSize.id === size.id ? "bg-foreground/10" : "bg-background/10"
                            }`}
                            style={{
                              width: 20 + index * 8,
                              height: 26 + index * 10,
                          ...
                          <div className="text-left">
                            ...
                            <p
                              className={`text-sm ${selectedSize.id === size.id ? "text-foreground/60" : "text-background/40"}`}
                            >
                              {size.dimensions}
                            </p>
                          </div>
              ...
              {activeTab === "text" && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                  ...
                  <p className="text-background/60 mb-6">Personalize your poster with a custom caption or title.</p>
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm text-background/60 mb-2 block">Your Text</label>
                      <input
                        type="text"
                        value={customText}
                      ...
                      />
                      <p className="text-xs text-background/40 mt-2">{customText.length}/50 characters</p>
                    </div>
                    <div>
                      <label className="text-sm text-background/60 mb-3 block">Text Position</label>
                      <div className="flex gap-3">
                        {[
                          ...
                          <button
                            key={option.value}
                            onClick={() => setTextPosition(option.value as typeof textPosition)}
              ...
              <div className="flex items-center justify-between">
                {tabs.map((tab, index) => (
                  <div key={tab.id} className="flex items-center">
                    ...
                    >
                      {tabs.findIndex((t) => t.id === activeTab) > index ? <Check className="w-4 h-4" /> : index + 1}
                    </div>
                    {index < tabs.length - 1 && (
                      <div
                        className={`w-12 md:w-20 h-0.5 mx-2 transition-all ${
                          tabs.findIndex((t) => t.id === activeTab) > index ? "bg-background" : "bg-background/20"
                        }`}
                      />
                    )}
                  </div>
