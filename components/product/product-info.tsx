...
import { ShoppingBag, Heart, Share2, Check, Truck, Shield, RotateCcw } from "lucide-react"
...
export function ProductInfo({ productId, title, price, category, description, sizes, details }: ProductInfoProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-balance">{title}</h1>
        <p className="text-2xl md:text-3xl font-light">${currentPrice}</p>
      </div>
          ...
          {sizes.map((size, index) => (
            ...
            >
              ...
              <span
                className={cn(
                  "block text-sm mt-1",
                  selectedSize === index ? "text-background/70" : "text-muted-foreground",
                )}
              >
                ${size.price}
              </span>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-muted-foreground">Total: ${currentPrice * quantity}</span>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <Button
          size="lg"
          className={cn("flex-1 h-14 text-base transition-all", isAdded && "bg-green-600 hover:bg-green-600")}
        ...
        <Button
          ...
          className={cn(
            ...
            isWishlisted && "bg-foreground text-background hover:bg-foreground/90",
          )}
