...
import { getOrderById } from "@/actions/user/checkout-actions";
import { Loader2, Printer, Scissors, ArrowLeft } from "lucide-react";
              ...
              {order.items.map((item: any, i: number) => (
                <tr key={i}>
                  <td className="py-3 font-bold uppercase">
                    {item.poster?.title ||
                      (item.customPosterId ? "Custom Poster" : "Art Print")}
                  </td>
        ...
        <div className="mt-12 flex items-center justify-between rounded-lg bg-gray-50 p-6 border border-gray-200">
          <div className="space-y-1">
            ...
            <p className="text-xs font-medium max-w-[300px]">
              Handle with care. Do not bend. Water-resistant packaging
              recommended for the posters inside.
            </p>
          </div>
          <div className="text-right">
            {order.paymentStatus === "paid" || order.paymentMethod !== "COD" ? (
